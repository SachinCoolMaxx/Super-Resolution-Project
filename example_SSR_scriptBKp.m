...
data_dir = '/iacl/pg15/amod/SelfSuperRes_Experiments/data/NeuroMM/imgs/cropped/2mm/'
...
for iter = 1:1% length(data_list)
    ...
    for witer = 1%:3
      ...
      %  for p = [0.05, 0.1, 0.2, 0.8, 1.2, 2,4,8,12]
       for p = [35]
            S2_HR = funcCombineAllFourierOrientations_standardMax(S2_r, all_axes, p,sz,maxdiff);
            ...
            outfile = [results_dir,subjname, '_varPatchsize_ANR_truncSinc_FinalSuperRes_pMaximum(p=infi)',num2str(p),'_3dpatches_withHROldnewrotation',num2str(window),'_9dirOrderChanged_.nii'];
            ...
            fg_idxs = find(A_HR > 375); % removes background and very dim voxels
