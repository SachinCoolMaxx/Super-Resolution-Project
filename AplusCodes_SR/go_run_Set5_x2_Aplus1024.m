...
p = '/home/amod/mimecs/code/code_basic_libs/AplusCodes_SR/';
...
input_dir = [p,'Set5']; % Directory with input images from Set5 image dataset
...
for d=10    %1024
    ...
    train_now = 1
    if train_now == 1 %exist([mat_file '.mat'],'file')
        ...
        startt = tic;
        data_dir = '/home/amod/mimecs/code/code_basic_libs/AplusCodes_SR/';
        conf = learn_dict(conf, load_images(...            
            glob([data_dir,'CVPR08-SR/Data/Training'], '*.bmp') ...
            ), dict_sizes(d));       
...
%Aplus_PPs{i} = Hi*(inv(Lo*Lo'+llambda*eye(size(Lo,1)))*Lo)'; 
    ...
    res =[];
    for i = 1%1:numel(conf.filenames)
        f = conf.filenames{i};
        [p, n, x] = fileparts(f);
