function [ im ] = rotImg3o( img, teta, ax, method, pad )
...
% if (ax == [1,0,0])
%     im = RotAboutx(img,teta);
%     return;
% end
% 
% if (ax == [0,1,0])
%     im = RotAbouty(img,teta);
%     return
% end
% 
% if(ax == [0,0,1])
%     im = RotAboutz(img,teta);
%     return
% end
...
if pad
      imagepad = padarray(img,[maxdiff,maxdiff,maxdiff]);
%     s = max(sz);
